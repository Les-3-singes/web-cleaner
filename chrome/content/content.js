...
async function loadEditModeState() {
  const data = await chrome.storage.local.get(['editModeEnabled']);
...
async function handleOverlayClick(e) {
  ...
  if (selector) {
    ...
    // Notifier le popup
    chrome.runtime.sendMessage({ type: 'ELEMENT_MASKED' }).catch(() => {});
...
async function undoLastAction() {
  ...
  // Notifier
  chrome.runtime.sendMessage({ type: 'ELEMENT_MASKED' });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'TOGGLE_EDIT_MODE') {
    ...
    chrome.storage.local.set({ editModeEnabled: editModeEnabled }, () => {
